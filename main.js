...
function addTask() {
  ...
  if (taskText != "") {
    ...
    window.alert("Please enter a task, that is not empty. ");
  }
}
...
function createNewTask(taskText) {
  ...
  checkbox.addEventListener("change", function () {
    completingTasks(newTask, checkbox.checked);
    saveData();
  });
...
// MOVING TASKS:
function completingTasks(newTask, completed) {
...
function sortingTasks() {
  ...
  var activeTasksSortable = new Sortable(activeTasksList, {
    group: "tasks",
    animation: 300,
  ...
  var completedTasksSortable = new Sortable(completedTasksList, {
    group: "tasks",
    animation: 300,
    onUpdate: function (event) {
      saveReorderedTasks(event.from.id, event.to.id);
